...
/**
 * A class that implements our simple List interface using a linked list. The
 * linked list includes a dummy head node that allows us to avoid special cases
 * for insertion and deletion at the front of the list.
 */
public class LLList implements List {
    ...
    /*
     * getNode - private helper method that returns a reference to the ith node
     * in the linked list. It assumes that the value of the parameter is valid.
     * 
     ...
     * items that are currently in positions i, i+1, i+2, etc. to the right by
     * one. Always returns true, because the list is never full.
     * 
     * We don't need a special case for insertion at the front of the list (i ==
     * 0), because getNode(0 - 1) will return the dummy head node, and the rest
     * of insertion can proceed as usual.
     */
    ...
    /**
     * removeItem - removes the item at position i in the list, shifting the
     * items that are currently in positions i+1, i+2, etc. to the left by one.
